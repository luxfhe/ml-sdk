...
This document introduces the simple built-in neural networks models that TorusML provides with a scikit-learn interface through the `NeuralNetClassifier` and `NeuralNetRegressor` classes.
...
The folowing figure shows the TorusML neural network trained with Quantization Aware Training in an FHE-compatible configuration and compares it to the floating-point equivalent trained with scikit-learn.
