...
Two types of ciphertext formats are [available in TorusML](../getting-started/concepts.md#ciphertext-formats) and both are available for deployment. To use the _Lux-FHE radix_ format, pass the `ciphertext_format` option to the compilation call as follows:
