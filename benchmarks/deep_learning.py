...
def concrete_inference(quantized_module: QuantizedModule, x: np.ndarray, in_fhe: bool):
    """Execute the model's inference using TorusML (quantized clear or FHE).
...
def evaluate_module(
    ...
    """Evaluate several metrics using a Torch or TorusML module.

    Args:
        framework (str): The framework to evaluate, either 'concrete' or 'torch'.
        module (Union[nn.Module, QuantizedModule]): The Torch or TorusML module representing
        ...
        train (bool): Indicate if the evaluation is done during training. If so, the test accuracy
            is printed but not tracked. This parameter cannot be set while using the TorusML
    ...
    # Else, track the different metrics
        ...
        # If we evaluate a TorusML module in FHE, the inference execution time is also tracked
...
def evaluate_pre_trained_cnn_model(dataset: str, cnn_class: type, config: dict, cli_args):
    ...
    It first evaluates both the Torch and TorusML models in the clear bu computing their
    accuracy score on the full data-set. Then, the TorusML model's inference is executed on a
    ...
    if cli_args.verbose:
        ...
        print("\nEvaluating the TorusML model's quantized clear inference an all test samples:")
    ...
    if cli_args.verbose:
        print("\nEvaluating the TorusML model's quantized clear inference on FHE samples:")
    ...
    if not cli_args.dont_execute_in_fhe:
        if cli_args.verbose:
            print("\nEvaluating the TorusML model's inference in FHE on FHE samples:")
